...
exports.create = (req,res)=>{
    ...
    const hospital = new Hospital({
      ...
      dgidIdName: req.body.dgidIdName,
      wgs84Lon: req.body.wgs84Lon,
      wgs84Lat: req.body.wgs84Lat,
      dutyEryn: req.body.dutyEryn || false,
      o038: req.body.o038,
...
exports.findAll = (req,res)=>{
    ...
    Hospital.getAll(req.query, (err, data) => {
        if (err) {
          res.status(500).send({
            message:
              err.message || "hospital 테이블을 검색하는 동안 에러가 발생했습니다."
          });
        }
        else {
          //console.log(data);
...
exports.findOne = (req,res)=>{
    ...
    Hospital.findById(req.params.hpid, (err, data) => {
        if (err) {
          if (err.kind === "not_found") {
            res.status(404).send({
              message: `Hospital 테이블에서 ${req.params.hpid}가 검색되지 않았습니다.`
            });
          } else {
            res.status(500).send({
  ...
  Hospital.updateById(
    ...
    (err, data) => {
      if (err) {
        if (err.kind === "not_found") {
          res.status(404).send({
            message: `hospital 테이블에서 hpid가 ${req.params.hpid}인 hospital이 존재하지 않습니다.`
          });
        } else {
          res.status(500).send({
            message:"hospital 테이블의 hyid가 " + req.params.hpid + " 인 업데이트하는 중에 에러 발생했습니다."
          });
        }
      } else res.send(data);
    }
...
exports.delete = (req,res)=>{
    Hospital.remove(req.params.hpid, (err, data) => {
        if (err) {
          if (err.kind === "not_found") {
            res.status(404).send({
              message: `Hospital 테이블에서 hpid인 ${req.params.hpid}가 없습니다.`
            });
          } else {
            res.status(500).send({
              message:  req.params.hpid + "인 hospital을 삭제할 수 없습니다. "
            });
          }
        } else res.send({ message: `hpid가 ${req.params.hpid}인 hospital이 삭제되었습니다.` });
    });
};
...
exports.deleteAll = (req,res)=>{
    Hospital.removeAll((err, data) => {
        if (err)
          res.status(500).send({
            message:
              err.message || "모든 hospital 데이터를 삭제하는 중에 에러가 발생했습니다."
          });
        else res.send({ message: `모든 hospital 데이터가 삭제되었습니다.` });
    });
};